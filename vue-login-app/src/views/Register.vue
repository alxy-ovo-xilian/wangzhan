    ...
    <div class="login-wrapper">
        <div class="login-container">
            ...
            <form @submit.prevent="handleRegister" class="floating-form">
                ...
                <div class="input-group">
                    <input id="nickname" v-model.trim="registerForm.nickname" type="text" autocomplete="off" @input="validateInput" required />
                ...
                <div class="captcha-section" v-if="showCaptcha">
                    <div class="input-group captcha-input">
                        <input id="captcha" v-model.trim="registerForm.captcha" type="text" maxlength="4" autocomplete="off" required />
...
// 输入验证
const validateInput = () => {
    if (registerForm.username && registerForm.email && registerForm.nickname && 
        ...
        if (registerForm.password.length < 5) {
            isFormValid.value = false
            errorMsg.value = '密码长度不能少于5位'
            return
        }
        
        if (showCaptcha.value && !registerForm.captcha) {
            isFormValid.value = false
            return
        }
...
const handleRegister = async () => {
    ...
    try {
        // 基本的安全检查
        const xssPattern = /[<>]/g
        if (xssPattern.test(registerForm.username) || xssPattern.test(registerForm.email) || 
            xssPattern.test(registerForm.nickname) || xssPattern.test(registerForm.password)) {
